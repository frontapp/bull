...
var MINIMUM_REDIS_VERSION = '2.8.6';
...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  // Flag to soft pause the queue (allows to pause the current Queue instance)
  // Set using queue.softPause()
  this.stopProcessing = false;

  // Shared redis client might have been defined in options
  this.client = (opts && opts.sharedClient) || (this.redisOptions && this.redisOptions.sharedClient);

  //
  // Create queue client (used to add jobs, pause queues, etc);
  //
  this.client = this.client || redis.createClient(this.redisPort, this.redisHost, this.redisOptions);

  // If client comes from options, promisify it
  ...
  this.client.on('error', _this.emit.bind(_this, 'error'));

  this.client.selectAsync(this.redisDB).then(function(){
    _this.emit('ready');
  });
...
Queue.prototype.initBlockingClients = function(){
  var _this = this;

  //
  // Create blocking client (used to wait for jobs)
  //
  this.bclient = redis.createClient(this.redisPort, this.redisHost, this.redisOptions);

  //
  // Create event subscriber client (receive messages from other instance of the queue)
  //
  this.eclient = redis.createClient(this.redisPort, this.redisHost, this.redisOptions);

  [this.bclient, this.eclient].forEach(function (client) {
    client.on('error', _this.emit.bind(_this, 'error'));
  });

  // emit ready when redis connections
  this.bclient.selectAsync(this.redisDB).then(function(){
    return _this.eclient.selectAsync(_this.redisDB);
  }).then(function(){
...
Queue.prototype.close = function(){
  ...
  return new Promise(function(resolve) {
    ...
    var triggerEvent = _.after(nbAttempts, resolve);
...
Queue.prototype.softPause = function () {
...
Queue.prototype.softResume = function () {
  if(!this.stopProcessing) {
    return Promise.resolve();
  }
