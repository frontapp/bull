...
describe('Queue', function(){
  ...
  describe(".soft pause", function(){
    it('should pause queue processing', function(done){
      queue = buildQueue();

      queue.process(function(job, jobDone){
        expect(job.data.foo).to.be.equal('pause_after_me');
        jobDone();
      });

      queue.on('stop_processing', function(){
        queue.getWaiting().then(function(jobs){
          expect(jobs.length).to.be.equal(1);
          expect(jobs[0].data.foo).to.be.equal('not_processed');
          done();
        });
      });

      queue.add({foo: 'pause_after_me'})
        .then(function () {
          return queue.softPause();
        })
        .then(function () {
          queue.add({foo: 'not_processed'});
        });
    });

    it('should resume queue processing', function(done){
      queue = buildQueue();

      queue.process(function(job, jobDone){
        expect(job.data.foo).to.be.equal('first');
        jobDone();
      });
